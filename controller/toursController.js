...
const multerFilter = (req, file, cb) => {
  if (file.mimetype.startsWith("image")) {
    ...
    cb(new AppError("wrong file type! please upload only images", 400), false);
  }
};
...
exports.resizeTourImages = catchAsync(async (req, res, next) => {
  ...
  await sharp(req.files.imageCover[0].buffer) // multplie files => array
    .resize(2000, 1333)
    .toFormat("jpeg")
    .jpeg({ quality: 90 })
...
exports.tourMonthlyPlan = catchAsync(async (req, res, next) => {
  ...
  const plan = await Tour.aggregate([
    {
      $unwind: "$startDates" // Deconstruct the document based on the startDates
    },
    {
      $match: {
        startDates: {
    ...
    },
    {
      $group: {
        ...
        tourName: { $push: "$name" }, //$push adds result in an array
        tourNumber: { $sum: 1 }
      }
    },
    {
...
exports.getToursWithin = catchAsync(async (req, res, next) => {
  ...
  if (!lat || !lng) {
    next(new AppError("please enter the distance in format lat,lng", 400));
  }
...
exports.getDistances = catchAsync(async (req, res, next) => {
  ...
  const [lat, lng] = latlng.split(",");
  // setting distance to killometer or milles
  const multipler = unit === "mi" ? 0.000621371 : 0.001;

  if (!lat || !lng) {
    next(new AppError("please provide distance in lat,lng format", 400));
  }
